...
J.vec = c(rep(50,2), rep(100,2), rep(150,2), rep(200,2))
...
sim1 <- function(N, J, K){
  ...
  A.matr <- matrix(runif(J*(K+1),-0.8,0.8), J, K+1);                 
...
for(i in 1:8){
  ...
  for(j in 1:3){
    ...
    n = sum(Omega==0)
    jml.res1 = confirm_CJMLE_poisson_cpp(resp,  nonmis_ind, theta0[,1:2], A0[,1:2], matrix(TRUE, J, 2), C =5, tol = .1/n)
    jml.res2 = confirm_CJMLE_poisson_cpp(resp,  nonmis_ind, theta0[,1:3], A0[,1:3], matrix(TRUE, J, 3), C =5, tol = .1/n)
    jml.res3 = confirm_CJMLE_poisson_cpp(resp,  nonmis_ind, theta0[,1:4], A0[,1:4], matrix(TRUE, J, 4), C =5, tol = .1/n)
    jml.res4 = confirm_CJMLE_poisson_cpp(resp,  nonmis_ind, theta0[,1:5], A0[,1:5], matrix(TRUE, J, 5), C =5, tol = .1/n)
    jml.res5 = confirm_CJMLE_poisson_cpp(resp,  nonmis_ind, theta0[,1:6], A0[,1:6], matrix(TRUE, J, 6), C =5, tol = .1/n)
...
filename  = paste("sim4res", jobid, ".Rdata", sep="")
